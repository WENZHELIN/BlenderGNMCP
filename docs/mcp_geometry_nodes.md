...
## Transport Decision (Initial Release)
- **Primary channel: stdio** under Blender's embedded Python to minimize socket complexity, match the MCP reference client, and avoid firewall/user prompts.
- **Fallback/WebSocket** can be added later for remote drivers; prefer authenticated loopback with explicit user opt-in and clear status in the UI.
- **Security notes**: sandbox payloads, validate method names against a registry, reject filesystem/network side effects, and log all requests/responses with redaction for tokens or file paths.
