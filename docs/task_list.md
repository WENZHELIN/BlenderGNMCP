...
- [x] 核對 MCP 通訊需求並決定初版傳輸方式（stdio 或 WebSocket），給出選擇理由與安全性注意事項。
- [x] 建立 Blender 外掛骨架：`__init__.py`、註冊/反註冊、偏好設定與日誌基礎設計。
- [x] 實作 MCP 請求路由與能力宣告（list_capabilities），包含參數模式與錯誤回應格式。
